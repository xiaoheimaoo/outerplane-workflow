...
public final class ConcurrentLinkedListKt {
    ...
    public static final <S extends Segment<S>> Object findSegmentInternal(S s, long j, Function2<? super Long, ? super S, ? extends S> function2) {
        while (true) {
            if (s.id >= j && !s.isRemoved()) {
                return SegmentOrClosed.m2101constructorimpl(s);
            }
            Object nextOrClosed = s.getNextOrClosed();
            if (nextOrClosed == CLOSED) {
                return SegmentOrClosed.m2101constructorimpl(CLOSED);
            }
    ...
    public static final boolean moveForward$atomicfu(Object obj, AtomicReferenceFieldUpdater atomicReferenceFieldUpdater, Segment segment) {
        while (true) {
            ...
            if (UByte$$ExternalSyntheticBackport0.m(atomicReferenceFieldUpdater, obj, segment2, segment)) {
    ...
    public static final boolean moveForward$atomicfu$array(Object obj, AtomicReferenceArray atomicReferenceArray, int i, Segment segment) {
        while (true) {
            ...
            if (UByte$$ExternalSyntheticBackport0.m(atomicReferenceArray, i, segment2, segment)) {
                if (segment2.decPointers$kotlinx_coroutines_core()) {
                    segment2.remove();
                }
                return true;
    ...
    public static final Object findSegmentAndMoveForward$atomicfu(Object obj, AtomicReferenceFieldUpdater atomicReferenceFieldUpdater, long j, Segment segment, Function2 function2) {
        ...
        do {
            ...
            while (true) {
                Segment segment2 = (Segment) atomicReferenceFieldUpdater.get(obj);
                z = true;
                if (segment2.id < m2104getSegmentimpl.id) {
                    if (!m2104getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        z = false;
                        continue;
                        break;
                    } else if (UByte$$ExternalSyntheticBackport0.m(atomicReferenceFieldUpdater, obj, segment2, m2104getSegmentimpl)) {
                        if (segment2.decPointers$kotlinx_coroutines_core()) {
                            segment2.remove();
                            continue;
                        } else {
            ...
            while (true) {
                ...
                if (segment2.id < m2104getSegmentimpl.id) {
                    if (!m2104getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        ...
                        continue;
                        break;
                    } else if (UByte$$ExternalSyntheticBackport0.m(atomicReferenceArray, i, segment2, m2104getSegmentimpl)) {
                        if (segment2.decPointers$kotlinx_coroutines_core()) {
                            segment2.remove();
                            continue;
                        } else {
